...
<template>
  <div
    class="bg-white rounded-[16px] shadow-[0_4px_24px_rgba(0,0,0,0.03)] overflow-hidden flex flex-col"
  >
    <div v-if="loading && !data" class="p-6 flex flex-col gap-4">
      <div
        v-for="i in 5"
        :key="i"
        class="h-10 bg-[rgba(148,151,169,0.12)] rounded-lg animate-pulse w-full"
      ></div>
    </div>
    ...
    <div v-else-if="data" class="overflow-x-auto w-full custom-scrollbar">
      <table class="w-full text-left border-collapse min-w-[600px]">
        <thead>
          <tr>
            <th
              class="sticky left-0 bg-[#f9fafc] p-4 text-xs font-semibold text-cool-gray uppercase tracking-wider border-b border-border-gray shadow-[1px_0_0_#dedee5] z-10 w-[200px]"
            >
              Member
            </th>
            <th
            ...
            <th
              class="p-4 text-xs font-semibold text-cool-gray uppercase tracking-wider border-b border-border-gray w-16 text-center"
            >
              Action
            </th>
          </tr>
        ...
        <tbody>
          ...
          >
            <td
              class="sticky left-0 p-4 shadow-[1px_0_0_#dedee5] z-10 bg-white group-hover:bg-[#f6f6f9] transition-colors"
            >
              <div class="flex items-center gap-3">
                ...
                />
                <div>
                  <div class="text-sm font-semibold text-near-black leading-tight">
                    {{ member.name }}
                  </div>
            ...
            <td v-for="col in member.columns" :key="col.column_name" class="p-4 whitespace-nowrap">
              <div class="text-sm text-cool-gray">
                <span class="font-bold" :class="col.task_count > 0 ? 'text-near-black' : ''">{{
                  col.task_count
                }}</span>
