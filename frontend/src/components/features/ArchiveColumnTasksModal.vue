  ...
  >
    <div class="flex flex-col h-[500px] max-h-[70vh]">
      ...
      <div class="shrink-0 mb-4 px-2 pt-2">
        <h3 class="text-xl font-bold text-on-surface mb-1 flex items-center gap-2 truncate" :title="column?.name">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar px-2 pb-2">
        <div v-if="loading" class="flex flex-col gap-3">
          <div v-for="i in 3" :key="i" class="h-16 animate-pulse bg-surface-container-low rounded-xl"></div>
        </div>

        <div v-else-if="error" class="h-full flex items-center justify-center p-6 text-center text-error bg-[var(--color-error)]/5 rounded-xl border border-error/20">
          {{ error }}
        </div>

        <div v-else-if="tasks.length === 0" class="h-full flex flex-col items-center justify-center text-center p-6 text-text-secondary bg-surface-container-lowest rounded-xl border border-border-gray">
        ...
        <div v-else class="flex flex-col gap-3">
          ...
          >
            <h4 class="font-semibold text-on-surface text-sm leading-tight group-hover:text-primary-container transition-colors line-clamp-2">
              {{ task.title }}
            </h4>
            <div class="flex items-center justify-between mt-1">
              <span class="flex items-center gap-1 text-[11px] font-medium text-neutral-gray bg-surface-container-low px-2 py-0.5 rounded-full">
                <CheckSquare :size="12" />
                {{ task.checklist_done_count }}/{{ task.checklist_total_count }}
              </span>
              
              <span v-if="task.deadline" class="flex items-center gap-1 text-[11px] font-medium text-text-secondary">
