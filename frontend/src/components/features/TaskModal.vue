...
<template>
  <Modal :model-value="isOpen" max-width="1000px" @update:model-value="emit('close')">
    <form
      ...
      id="task-form"
      class="flex flex-col md:flex-row gap-6 md:gap-8 min-h-[450px]"
    >
      <!-- Left Main Content -->
      <div class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          >
            <MarkdownEditor
              v-model="content"
              placeholder="Add a description to explain the task requirements..."
              class="flex-1 h-full min-h-[300px]"
            />
          </div>
      ...
      >
          ...
          <div class="flex flex-wrap gap-2 items-center">
            ...
            <div class="relative" ref="assigneesDropdownRef">
              ...
              <Transition name="t-dropdown">
                <div
                  v-if="isAssigneesDropdownOpen"
                  class="absolute top-full mt-2 left-0 md:right-0 md:left-auto origin-top-left md:origin-top-right w-64 bg-white border border-border-gray rounded-xl shadow-dropdown z-50 py-2 max-h-60 overflow-y-auto custom-scrollbar"
                >
                  <div
                    v-if="boardsStore.membersLoading"
                    class="px-4 py-3 text-sm text-text-secondary"
                  >
                    Loading members...
                  </div>
                  <div
                    v-else-if="assignableMembers.length === 0"
                    class="px-4 py-3 text-sm text-text-secondary"
                  >
                  ...
                  >
                    ...
                    <Check
                      v-if="assignees.includes(member.username)"
                      :size="16"
        ...
        <div>
          ...
          <div class="space-y-3">
            ...
            />
            <TransitionGroup name="t-list" tag="div" class="flex flex-wrap gap-2">
              <div
                v-for="tag in tags"
                :key="tag"
